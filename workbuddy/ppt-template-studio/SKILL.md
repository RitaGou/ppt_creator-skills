...
运行解析器时，把当前任务根目录传为 `workspaceRoot`（CLI 为 `--workspace-root "<工作区>"`），普通用户无需提供模板库路径；只有明确要用其他模板库时才传 `libraryRoot`。
...
解析器返回 `confirmation` 时，只询问其中一个类别；收到“保持”或“可变”后，把答案写入该类别的 `boundaryOverride` 再继续，不要同时追问其他边界。
